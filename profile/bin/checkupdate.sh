...
checkit()
{
	...
	then
		...
		then
			if [ -n "$PATTERN" ]
			then
				cat $RAWDATA|sed 's/width/widta/'|w3m -O big5 -dump -T  text/html -cols 1000|grep -e "$PATTERN" |tr -s " ">$NOWLIST
			else
...
add_data "GPS9900" "http://www.tw-radar.com.tw/tech/file/upfile_list.asp" "GPS.*9900.*台灣"
add_data "GPS9968" "http://www.5685.com.tw/default.php" "台灣版"
add_data "Mobile01" "http://www.mobile01.com/downloads.php" "更新日"
