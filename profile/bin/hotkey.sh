...
case "$3" in
	...
	0000004c)	eject /dev/cdrom
			;;
	00000050)	[ -n "$user" ] && su - $user -c "export DISPLAY=:0.0;/usr/bin/thunderbird  &"
			;;
	00000051)	[ -n "$user" ] && su - $user -c "export DISPLAY=:0.0;/usr/bin/firefox &"
			;;
	0000005c)	/etc/acpi/changespeed.sh &
			;;
