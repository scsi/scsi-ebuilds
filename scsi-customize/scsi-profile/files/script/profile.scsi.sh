...
if [ "`id -u`" = 0 ]; then
	...
	alias gentooupdate_no_distcc='time (date;layman -S;eix-sync ;date;hostname;FEATURES=-distcc MAKEOPTS=-j`nproc` emerge -uDNv world;emerge @preserved-rebuild;emerge --depclean -pq)'
fi
